...
int isValid(char* lang)
{
   return (strcmp(lang, "it") == 0)
      || (strcmp(lang, "en") == 0)
      || (strcmp(lang, "fr") == 0)
      || (strcmp(lang, "es") == 0)
      || (strcmp(lang, "de") == 0)
...
for (i=0,j=0; read(s2,hbuffer+i,1); i++) {
  ...
  if (hbuffer[i]==':' && !h[j].v && j>0){
	...
	if ( !strcmp(method,"GET")){
      ...
      for (j=0; h[j].n[0]; j++)
      {
         if (strcmp(h[j].n, "Accept-Language") == 0)
         {
            printf("Value: %s\n", h[j].v);
            found = 1;
      ...
      char* chosen = "en"; // default
      double max=0;

      // Pick the max value
      for (i=0; i <= k; i++)
      {
         printf("lang %d:%s, qvalue:%f\n", i, l[i].n, l[i].vd);
         if (l[i].vd > max && isValid(l[i].n))
         {
            chosen = l[i].n;
            max = l[i].vd;
         }
      ...
      // Adapt the request if the header Accept-Language is found
      if (found)   
      {
		   sprintf(request,"GET /%s/%s HTTP/1.1\r\nHost:%s\r\nConnection:close\r\n\r\n", chosen, filename, hostname);
      }
      else
      {
