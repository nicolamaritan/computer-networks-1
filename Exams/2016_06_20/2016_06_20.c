...
char* forb1 = "127.0.0.2";
char* forb2 = "11.11.11.11";
char* forb3 = "22.22.22.22";
char* forb4 = "33.33.33.33";
int forbidden = 0;
...
int main()
{
		  ...
		  while ( 1 ){
					 ...
					 // Reset for parent process
					 forbidden = 0;
					 // Get IP address from struct sockaddr_in
					 char* ptr = (char*)&remote.sin_addr.s_addr;
					 char ip[11];
					 sprintf(ip, "%d.%d.%d.%d", ptr[0], ptr[1], ptr[2], ptr[3]);
					 printf("IP: %s\n", ip);

					 if ((strcmp(ip, forb1) == 0) ||
									 (strcmp(ip, forb2) == 0) ||
									 (strcmp(ip, forb3) == 0) ||
									 (strcmp(ip, forb4) == 0))
					 {
						printf("Answers only with html/text\n");
						forbidden = 1;
					 }
					 ...
					 if ( !strcmp(method,"GET")){
								...
								if (forbidden)
								{
										...
										for (j=0; h[j].n[0]; j++)
										{
												if (strcmp(h[j].n, "Content-Type") == 0)
												{
														if((strcmp(h[j].v+1, "text/html") != 0) && (strcmp(h[j].v+1, "text/plain") != 0))
														{
