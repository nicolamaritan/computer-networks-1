...
int main()
{
		...
		while ( 1 ){
				...
				if ( !strcmp(method,"GET")){
						...
						// Request Connection:keep-alive to the server
						...
						// Send headers to client, but filters Connection header
						char* keepAliveVal = "keep-alive";
						char* closeVal = "close";
						int foundConnection = 0;
						for (j=1; h[j].n[0]; j++)
						{
							write(s2, h[j].n, strlen(h[j].n));
							write(s2, ":", 1);
							if (strcmp(h[j].n, "Connection") == 0)
							{
								// Sets to close
								foundConnection = 1;
								write(s2, closeVal, strlen(closeVal));
							}
							else
							{
								// Write header value from the server
								write(s2, h[j].v, strlen(h[j].v));
							}
							write(s2, "\r\n", 2);
						}
						if (!foundConnection)
						...
						// Continue instead of closing connection as keep-alive is requested
						//close(s3);
						continue;
				}
