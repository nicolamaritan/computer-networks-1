...
int main(){
	...
	while( 1 ){
		...
		// Search SID in the string if cookie is not empty
		int len = strlen(cookie);
		if (len > 0)
		{
			ptr = strstr(cookie, "SID=");
			sid=ptr+4;
		...
		if (err) 
			...
			if(!strcmp(method,"GET")){
				...
				if ((fin = fopen(filename+1,"rt"))==NULL)
					...
					if (!strcmp(filename, "/file1.html"))
					{
						sprintf(response,"HTTP/1.1 200 OK\r\nSet-Cookie:SID=file1cookie\r\n\r\n");
						write(s2,response,strlen(response));
						while( EOF != (ch=fgetc(fin))){
							write(s2,&ch,1);
						}
					}
					else if (!strcmp(filename, "/file2.html"))
					{
						if (!strcmp(sid, "file1cookie"))
						{
							sprintf(response,"HTTP/1.1 200 OK\r\nSet-Cookie:SID=file1cookie; Expires=Wed, 02 Jan 1970 00:00:00 GMT\r\n\r\n");
							write(s2,response,strlen(response));
							while( EOF != (ch=fgetc(fin))){
								write(s2,&ch,1);
							}
